...
if(config.txconverter) {
  Rpc.getTxInfo = function(txid, doNotParse, cb) {
    ...
    request.get({
      url: config.txconverter + txid,
      json: true
    }, function (err, res, result) {
      if (!err) {
        if(result.err) {
          if(result.err.code === -5) {
            return cb();
          } else {
            return cb(self.errMsg(result.err));
          }
        }
      } else {
      ...
      var info = doNotParse ? result.res : self._parseTxResult(result.res);
      return cb(null,info);
    });
  };
} else {
